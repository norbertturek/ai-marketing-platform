...
2. **Run Playwright e2e tests** — ensure full-stack flows work: `pnpm docker:up` (if not running), then `pnpm test:e2e`. If e2e fails, report as Major/Blocker.
3. **Trace connected dependencies** — check callers, imports, and downstream effects.
4. **Check project conventions** — verify patterns match the codebase:
...
5. **Enumerate findings** in severity order with concrete fix guidance.
6. **Call out missing tests** and required scenarios.
7. **State assumptions and open questions** explicitly.
8. **Provide residual risk summary.**
