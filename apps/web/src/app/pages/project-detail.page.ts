...
import { PostsApiService, PostResponse } from '../core/projects/posts-api.service';
    ...
    >
      <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p class="mt-1 text-sm text-zinc-500">Project posts</p>
        </div>
        <button
          hlmBtn
          type="button"
      ...
      @if (loading()) {
        ...
        <div class="rounded-lg border border-zinc-800/50 bg-zinc-900/30 p-12 text-center">
          <lucide-icon
            ...
            aria-hidden="true"
          ></lucide-icon>
          <h2 class="text-lg font-medium text-white">No posts</h2>
          <p class="mt-2 text-sm text-zinc-500 mb-6">Start by creating your first post.</p>
          <button
            hlmBtn
            type="button"
        ...
        <div class="space-y-4">
          @for (post of posts(); track post.id) {
            <a [routerLink]="['/project', projectId(), 'post', post.id]" class="block">
              <article
              ...
              >
                <div class="flex items-start justify-between gap-4">
                  <div class="flex min-w-0 flex-1 items-center gap-3">
                    ...
                    ></lucide-icon>
                    <div>
                      <h3 class="text-base font-medium text-white">Post</h3>
...
export class ProjectDetailPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (pid) {
      ...
      this.errorMessage.set('Project ID missing.');
    }
  }
  ...
  protected createPost(): void {
    ...
    this.postsApi.createPost(pid).subscribe({
      ...
      error: () => {
        this.errorMessage.set('Could not create post. Please try again.');
